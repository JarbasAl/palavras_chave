...
if system == "Linux":
    ...
    if machine == "x86_64":
        from palavras_chave.res.snowboy.lib.linux import snowboydetect
    elif machine == "armv6l":
        # TODO not sure this is functional!!!
        from palavras_chave.res.snowboy.lib.rpi import snowboydetect
    elif machine == "armv7l":
        from palavras_chave.res.snowboy.lib.rpi import snowboydetect
    else:
...
PKG_ROOT = dirname(dirname(os.path.abspath(__file__)))
RESOURCE_FILE = join(PKG_ROOT, "common.res")
...
def find_model(model):
    ...
    bundled_models_path = join(PKG_ROOT, "models")
...
DETECT_DING = os.path.join(PKG_ROOT, "ding.wav")
DETECT_DONG = os.path.join(PKG_ROOT, "dong.wav")
